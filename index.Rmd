...
p <- ggplot(patdat, aes(x=TOTPAID))
p + geom_histogram(breaks=seq(0, 10000, by=500)) + annotate("text", x = 8000, y = 500, label = paste("Total Patient Count = ", stats[1,2])) + annotate("text", x = 8000, y = 450, label = paste("Average Cost Per Patient = $",stats[1,1])) + labs(list(title = "Long Term Care Costs: Entire Sample", x = "Total Amount Paid", y = "Patient Count"))
